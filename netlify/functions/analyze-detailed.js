...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        // Updated system prompt for express analysis with client-focused approach
        const DETAILED_SYSTEM_PROMPT = `${langPrefix}\n` +
            ...
            `ЗАДАЧА: Провести ЭКСПРЕСС АНАЛИЗ образца с фокусом на клиентском опыте и практической пользе.\n\n` +
            
            `ТОН И ПОДАЧА:\n` +
            `- Дружелюбный и заботливый тон, как опытный консультант\n` +
            `- Делай мягкие акценты на важных находках: "Обратил внимание на...", "Стоит отметить...", "Важный сигнал..."\n` +
            `- Используй медицинскую терминологию в меру, когда она уместна, но сопровождай понятными объяснениями\n` +
            `- Подчеркивай значимость находок без запугивания: "Это частая особенность, которая указывает на..."\n` +
            `- ОБЪЕКТИВНОСТЬ ПРЕВЫШЕ ВСЕГО: не приукрашивай находки, честно описывай то, что видишь\n\n` +
            
            `АНАЛИЗ ДОЛЖЕН ВКЛЮЧАТЬ:\n` +
            `- Общее впечатление и ключевые визуальные характеристики\n` +
            `- Цветовые особенности с акцентом на отклонения от нормы\n` +
            `- Состояние поверхности, налеты, текстура\n` +
            `- Структурные элементы: сосочки, борозды, края\n` +
            `- Мягкие указания на проблемные зоны с объяснением их значения\n\n` +
            
            `ВАЖНО:\n` +
            `- Будь объективным в оценках - если есть проблемы, указывай на них корректно\n` +
            `- Каждую находку объясняй: медицинский термин + простое объяснение в скобках\n` +
            `- Используй фразы типа: "Ваш язык показывает...", "Заметно, что...", "Это говорит о том, что..."\n` +
            `- Пиши живым языком, но с профессиональной точностью\n\n` +
            
            `ОТВЕТ СТРОГО в JSON формате:\n` +
            `{\n` +
            `  "general_impression": "Дружелюбное общее впечатление о состоянии с указанием на ключевые моменты для внимания",\n` +
            `  "key_findings": "Основные визуальные находки с мягкими акцентами на их важности, объясненные простым языком с указанием что это означает для организма",\n` +
            `  "detailed_analysis": "Подробное описание всех характеристик в живом, заботливом тоне с акцентами на значимых особенностях",\n` +
            `  "health_indicators": "Что обнаруженные особенности говорят о состоянии организма, объясненное доступно и с заботой",\n` +
            `  "attention_points": "Моменты, которые стоит взять на заметку для поддержания здоровья, сформулированные мотивирующе"\n` +
            `}${langSuffix}`;
